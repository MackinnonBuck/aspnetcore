...
export module DotNet {
    ...
    const dotNetAppKey = "__dotNetApp";
    ...
    const dotNetDispatchersByAppId = new Map<number, DotNetCallDispatcher>();
     ...
     */
    export function attachDispatcher(dispatcher: DotNetCallDispatcher, appId = 0) {
        if (appId === 0 && dotNetDispatchersByAppId.size !== 0) {
            throw new Error("Cannot register a default DotNetCallDispatcher after a non-default dispatcher was already registered.");
        }

        if (appId !== 0 && dotNetDispatchersByAppId.has(0)) {
            throw new Error("Cannot register a DotNetDispatcher after a default dispatcher was already registered.");
        }

        dotNetDispatchersByAppId.set(appId, dispatcher);
    }
    ...
    export function invokeMethod<T>(assemblyName: string, appId: number, methodIdentifier: string, ...args: any[]): T;
    export function invokeMethod<T>(assemblyName: string, appIdOrMethodIdentifier: string | number, methodIdentifierOrFirstArg?: any, ...args: any[]): T {
        switch (typeof appIdOrMethodIdentifier) {
        case "string":
            return invokePossibleInstanceMethod(assemblyName, appIdOrMethodIdentifier, 0, null, [
                methodIdentifierOrFirstArg,
                ...args
            ]);
        default:
            return invokePossibleInstanceMethod(assemblyName, methodIdentifierOrFirstArg, appIdOrMethodIdentifier, null, args);
        }
    }
    ...
    export function invokeMethodAsync<T>(assemblyName: string, appId: number, methodIdentifier: string, ...args: any[]): Promise<T>;
    export function invokeMethodAsync<T>(assemblyName: string, appIdOrMethodIdentifier: string | number, methodIdentifierOrFirstArg?: any, ...args: any[]): Promise<T> {
        switch (typeof appIdOrMethodIdentifier) {
        case "string":
            return invokePossibleInstanceMethodAsync(assemblyName, appIdOrMethodIdentifier, 0, null, [
                methodIdentifierOrFirstArg,
                ...args
            ]);
        default:
            return invokePossibleInstanceMethodAsync(assemblyName, methodIdentifierOrFirstArg, appIdOrMethodIdentifier, null, args);
        }
    }
    ...
    function invokePossibleInstanceMethod<T>(assemblyName: string | null, methodIdentifier: string, appId: number, dotNetObjectId: number | null, args: any[] | null): T {
        const dispatcher = getRequiredDispatcher(appId);
        if (dispatcher.invokeDotNetFromJS) {
            const argsJson = stringifyArgs(args, dispatcher);
    ...
    function invokePossibleInstanceMethodAsync<T>(assemblyName: string | null, methodIdentifier: string, appId: number, dotNetObjectId: number | null, args: any[] | null): Promise<T> {
        ...
        try {
            const dispatcher = getRequiredDispatcher(appId);
            const argsJson = stringifyArgs(args, dispatcher);
            dispatcher.beginInvokeDotNetFromJS(asyncCallId, assemblyName, methodIdentifier, dotNetObjectId, argsJson);
        } catch (ex) {
            // Synchronous failure
    ...
    function getRequiredDispatcher(appId: number): DotNetCallDispatcher {
        if (dotNetDispatchersByAppId.size === 1) {
            // In the case where only one dispatcher was registered, return it no matter what.
            // This exists for backwards compatibility.
            return dotNetDispatchersByAppId.values().next().value;
        }

        const dispatcher = dotNetDispatchersByAppId.get(appId);
        if (dispatcher) {
            return dispatcher;
        }

        throw new Error(`No .NET call dispatcher has been set for the app with ID ${appId}.`);
    }
    ...
    export const jsCallDispatcher = {
         ...
         */
        invokeJSFromDotNet: (identifier: string, argsJson: string, resultType: JSCallResultType, targetInstanceId: number, appId = 0) => {
            ...
            return result === null || result === undefined
                ? null
                : stringifyArgs(result, dispatcher);
        },
         ...
         */
        beginInvokeJSFromDotNet: (asyncHandle: number, identifier: string, argsJson: string, resultType: JSCallResultType, targetInstanceId: number, appId = 0): void => {
            ...
            if (asyncHandle) {
                ...
                promise
                    .then(result => stringifyArgs([asyncHandle, true, createJSCallResult(result, resultType)], getRequiredDispatcher(appId)))
                    .then(
                        result => getRequiredDispatcher(appId).endInvokeJSFromDotNet(asyncHandle, true, result),
                        error => getRequiredDispatcher(appId).endInvokeJSFromDotNet(asyncHandle, false, JSON.stringify([
                            asyncHandle,
                            false,
                            formatError(error)
                        ]))
    ...
    export class DotNetObject {
        ...
        constructor(private _id: number, private _appId: number) {
        }
        ...
        public invokeMethod<T>(methodIdentifier: string, ...args: any[]): T {
            return invokePossibleInstanceMethod<T>(null, methodIdentifier, this._appId, this._id, args);
        }

        public invokeMethodAsync<T>(methodIdentifier: string, ...args: any[]): Promise<T> {
            return invokePossibleInstanceMethodAsync<T>(null, methodIdentifier, this._appId, this._id, args);
        }

        public dispose() {
            const promise = invokePossibleInstanceMethodAsync<any>(null, "__Dispose", this._appId, this._id, null);
            promise.catch(error => console.error(error));
        }

        public serializeAsArg() {
            return {
                [dotNetObjectRefKey]: this._id,
                [dotNetAppKey]: this._appId
            };
        }
    }
    ...
    attachReviver(function reviveReference(key: any, value: any) {
        if (value && typeof value === "object") {
            if (value.hasOwnProperty(dotNetObjectRefKey)) {
                return new DotNetObject(value[dotNetObjectRefKey], value[dotNetAppKey]);
    ...
    function stringifyArgs(args: any[] | null, dotNetDispatcher: DotNetCallDispatcher) {
